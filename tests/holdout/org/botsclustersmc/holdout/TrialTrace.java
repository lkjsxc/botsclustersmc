...
final class TrialTrace {
    ...
    private final int[] guiOperations=new int[6],clickedSlots=new int[64];
    ...
    String json(Npc npc) {
        ...
        return motor.substring(0,motor.length()-1)+String.format(Locale.ROOT,
            ",\"gui_operations\":%s,\"clicked_slots\":%s,\"menu_open_observations\":%d,\"recipe_visible_observations\":%d,\"max_grid_units\":%d,\"max_crafted_units\":%d,\"drop_decisions\":%d,\"use_decisions\":%d,\"minimum_raw_units\":%d,\"first_clicks_op_slot_menu_cursor_kind_count\":\"%s\"}",
            Arrays.toString(guiOperations),Arrays.toString(clickedSlots),openObservations,recipeObservations,
