...
public final class UpdateTest {
    ...
    public static void main(String[] args)throws Exception {
        ...
        for(boolean[] mask:new boolean[][]{Schema.unrestrictedMask(),Task.FORWARD_STOP.mask(0,false)}) {
            double[] p=new double[Schema.LOGITS];Distribution.probabilities(logits,mask,p);
            ...
            near(UpdateGuard.divergence(p,p),0,1e-12);
        }
        ...
        double[] before=new double[Schema.LOGITS],after=new double[Schema.LOGITS];int offset=0;
        ...
        near(UpdateGuard.divergence(before,after),0,1e-12); // GUI slot is inactive.
        after[child]=1;after[child+1]=0;near(UpdateGuard.divergence(before,after),0,1e-12);
        after[child]=.9;after[child+1]=.1;
        int parent=Task.offset(6);before[parent]=after[parent]=0;before[parent+1]=after[parent+1]=1;
        near(UpdateGuard.divergence(before,after),.5*Math.log(.5/.9)+.5*Math.log(.5/.1),1e-12);
