...
public final class Observatory implements CommandExecutor, TabCompleter, Listener {
    ...
    private void inspect(CommandSender sender,long id) {
        ...
        sender.sendMessage(s.pocket().summary());
        if(!s.pocket().cells().isEmpty())sender.sendMessage("Menu slots "+s.pocket().contents());
    ...
    private void tick(Player player,Watch w) {
        ...
        player.sendActionBar(Component.text("#"+w.actor+" "+s.task()+" | "+plugin.observerHud(w.actor)+(s.task().equals("break-log")||s.task().equals("collect-log")?" | dig "+s.miningTicks()+"t | broken "+s.broken()+" picked "+s.collected():"")+(s.pocket().cells().isEmpty()?"":" | "+s.pocket().menu()+" | cursor "+s.pocket().cursor().item()+" x"+s.pocket().cursor().count())+(age>2?" | STALE":"")));
