...
public final class Npc {
    public final RuntimePlugin plugin;public final long id;public final Mob entity;public final Pocket pocket=new Pocket();
    ...
    public volatile long decisions,lastStepNanos;public volatile AgentSnapshot snapshot;public volatile Frame observedFrame;public int stillTicks;private float lastYaw,lastPitch;public volatile double horizontalTravel;public volatile World currentWorld;public final Location anchor;
    public record Applied(Frame frame,InferencePool.Result result){}
    public Npc(RuntimePlugin plugin,long id,Mob entity,Goal goal){this.plugin=plugin;this.id=id;this.entity=entity;this.goal=goal;rng=new RandomSource(id*7919+plugin.seed);anchor=entity.getLocation().clone();currentWorld=anchor.getWorld();x=anchor.getX();y=anchor.getY();z=anchor.getZ();}
    ...
    private void step(){
        ...
        if(applied!=null&&tick>=nextDecision){
            Frame next=Sensors.capture(this);Applied completed=applied;applied=null;decisions++;plugin.transitions.increment();observedFrame=next;snapshot=AgentSnapshot.capture(this,next,completed);
    ...
    public void reset(Goal newGoal,Location target,Runnable supplies){
        ...
        entity.teleportAsync(target).whenComplete((ok,failure)->{
            ...
            entity.getScheduler().run(plugin,t->{goal=newGoal;episodeStart=tick;pocket.clear();broken.clear();collected.clear();placed.clear();mining=null;miningTicks=0;container=null;Arrays.fill(previousKinematics,0);stillTicks=0;supplies.run();Location at=entity.getLocation();x=at.getX();y=at.getY();z=at.getZ();currentWorld=at.getWorld();resetting=false;ready=true;status="acting";},()->plugin.fail(new IllegalStateException("NPC retired during reset")));
        });
    }
