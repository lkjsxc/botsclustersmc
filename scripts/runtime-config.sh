...
bcmc_validate_runtime() {
  ...
  [[ ${BOT_PREFIX:-} =~ ^[a-zA-Z0-9_]{1,13}$ ]] || {
    echo 'BOT_PREFIX must be 1..13 ASCII letters, digits or underscores. Use bcmc (bcmc00..bcmc31); botsclustersmc is too long for the pinned client.' >&2; return 1;
  }
  for name in BOTS SERVER_PORT JAVA_HEAP_GB FOLIA_THREADS MIN_FREE_GB RUN_SECONDS STARTUP_TIMEOUT_SECONDS; do
    value=${!name:-}
    [[ $value =~ ^[0-9]{1,9}$ ]] || { echo "$name must be an unsigned integer." >&2; return 1; }
    printf -v "$name" '%d' "$((10#$value))"; export "$name"
  done
  (( BOTS >= 1 && BOTS <= 32 )) || { echo 'BOTS must be 1..32.' >&2; return 1; }
