...
if [[ -f .botsclustersmc-academy-v1 ]]; then exec ./scripts/run.sh "$@"; fi
lab="$BCMC_ROOT/academy"
[[ ! -L $lab ]] || { echo 'Refusing a symlink academy/.' >&2; exit 1; }
[[ $BOTS =~ ^[0-9]+$ ]] && ((10#$BOTS >= 1 && 10#$BOTS <= 32)) || { echo 'BOTS must be 1..32' >&2; exit 1; }
BOTS=$((10#$BOTS)); export BOTS
marker=$(printf 'botsclustersmc-academy-v1\nbots=%s\ncampus=8x16\n' "$BOTS")
if [[ -e $lab ]]; then
  [[ -d $lab && -f $lab/.botsclustersmc-academy-v1 && ! -L $lab/.botsclustersmc-academy-v1 ]] || {
    echo 'Existing academy is not owned by this version. Preserve it and use a fresh source directory.' >&2; exit 1;
  }
  [[ $(cat "$lab/.botsclustersmc-academy-v1") == "$marker" ]] || {
...
if [[ ! -e $lab ]]; then mkdir -- "$lab"; printf '%s\n' "$marker" > "$lab/.botsclustersmc-academy-v1"; fi
...
done
{
  for key in BOTS BOT_PREFIX SEED BIND_ADDRESS SERVER_PORT OFFLINE_ACCESS_ACK JAVA_HEAP_GB FOLIA_THREADS \
      ...
      JAVA_BIN JAVAC_BIN FOLIA_BUILD ALLOW_EXPERIMENTAL_FOLIA CARGO_BUILD_JOBS BATCH_SAMPLES STARTUP_TIMEOUT_SECONDS; do
    if [[ -v $key ]]; then printf '%s=%q\n' "$key" "${!key}"; fi
  done
  printf '%s\n' 'BCMC_CURRICULUM=true' 'ACTION_HZ=5' 'ROLLOUT_STEPS=64'
} > "$lab/.env"
printf 'botsclustersmc Academy: %s (%s bots). Status/stop commands work from this source root.\n' "$lab" "$BOTS"
cd -- "$lab"
export BCMC_ROOT="$lab" BCMC_CURRICULUM=true ACTION_HZ=5 ROLLOUT_STEPS=64 BCMC_ACADEMY_LOCK=1
exec ./scripts/run.sh
