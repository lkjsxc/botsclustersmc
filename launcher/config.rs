...
impl Config{
    pub fn load()->Result<Self>{
        ...
        Ok(Self{port:number("SERVER_PORT",25565,1,65535)? as u16,curriculum:env::var("BCMC_CURRICULUM").as_deref()==Ok("true"),bind,heap:number("JAVA_HEAP_GB",6,2,8)?,threads:number("FOLIA_THREADS",6,1,10)?,bots:number("BOTS",32,1,32)?,view:number("VIEW_DISTANCE",3,3,8)?,sim:number("SIMULATION_DISTANCE",3,3,8)?,disk:number("MIN_FREE_GB",10,2,100)?,seconds:number("RUN_SECONDS",0,0,864000)?,mode})
    }
    pub fn local_address(&self)->String{format!("{}:{}",if self.bind.is_unspecified(){Ipv4Addr::LOCALHOST}else{self.bind},self.port)}
    pub fn write(&self,root:&Path)->Result<()>{
        if !self.curriculum || !root.join(".botsclustersmc-academy-v1").is_file(){return Err("Only the isolated Academy is supported; use ./start.sh".into());}
