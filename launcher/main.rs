...
fn main_inner()->Result<()>{
    ...
    if action=="status"{
        ...
        if let Ok(text)=fs::read_to_string(root.join("state/academy-status.json")) {
            ...
            println!("Academy stage={} ({}) phase={}",c.get("stage")?.integer()?,c.get("name")?.text()?,c.get("phase")?.text()?);
            println!("foundations_passed={:?}; a pass does NOT certify cooperative living",c.get("foundations_passed")?);
            for s in c.get("scores")?.array()? {
                println!("agent {}: training {}/40, exam current {}/{} (need 14/16), review {}/{} (need 3/4 for EACH earlier skill)",s.get("id")?.integer()?,s.get("trained")?.integer()?,s.get("exam_current_success")?.integer()?,s.get("exam_current")?.integer()?,s.get("exam_review_success")?.integer()?,s.get("exam_review")?.integer()?);
                println!("  retained successes by stage: {:?}",s.get("retained")?);
            }
        }
