...
use super::{campus,audit,install::Result,json,learning::{checkpoint::Checkpoint,curriculum::Curriculum}};
...
pub fn rows(text:&str,run:&str,bots:usize)->Result<Vec<usize>> {
    ...
    for line in text.lines() {
        ...
        if id>=bots||stage>=6||!seen.insert((id,serial)){return Err("duplicate or invalid academy episode identity".into());}
...
pub fn verify(root:&Path,previous:Option<&Path>)->Result<()> {
    ...
    if !root.join(".botsclustersmc-academy-v1").is_file(){return Err("not an isolated academy".into());}
    ...
    if !(1..=32).contains(&bots){return Err("invalid academy population".into());}
    ...
    let cp=Checkpoint::load(&root.join("state/policy.bcmc")).map_err(|e|e.to_string())?;
    let course=Curriculum::decode(&fs::read(root.join("state/academy.bcmc")).map_err(|e|e.to_string())?,bots,cp.model.version,cp.model.fingerprint()).map_err(|e|e.to_string())?;
    if course.stage as u64!=state.get("stage")?.integer()?{return Err("academy checkpoint/status stage mismatch".into());}
    ...
    println!("PASS: academy bridge, paired curriculum checkpoint and completed episode records checked: {counts:?}.");
